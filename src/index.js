/*
 * 装配者：全部顶层实例（MessageStore/NapCatClient/Summarizer/ChatBot/Scheduler/Analytics/DataRefresher，以及
 * 按 webui.enabled 条件装配的 WebUI）都在本文件 new，是唯一装配者（index ← 除 logger 外全部模块）；
 * 同时承载消息事件路由链（S1–S13，命中即 return，见本文件下部 onEvent）与三个后台编排流程：
 * refreshData（数据刷新 §6.3）/ backfillHistory（离线补偿 §6.1）/ dailyReport（每日日报 §6.2）。
 *
 * 模块级变量清单：实例 store/client/summarizer/chatBot/scheduler/analytics/refresher（+ 底部 WebUI）；
 * 配置派生常量 trackedGroups()/tracksGroup/minMessages/includeSelf/manualCmds、静默时段 quiet.*、filterEnabled/filterMessages、report.*；
 * 运行时状态 startedAt/wsConnected/selfId/ready/backfillDone/summaryInFlight；路径 configPath/dataDir。
 * 读 config.json（CONFIG_PATH 环境变量可覆盖；llm.apiKey 缺失回退 LLM_API_KEY，仍空则报错 exit(1)）；
 * 写经 store/analytics/arkdb 落 data/，回复经 client 发群/私聊消息。
 */
 ...
 * 先快照旧 6★/5★ 干员与卡池 id → refresher.refresh()（依次 干员→档案→藏品→卡池；ETag 304 跳过、结构校验失败抛错）
 * → 有更新则 arkdb.reload() 热重载并 diff 出新增干员/新开放卡池 → 按 dataRefresh.announce 组「【数据更新播报】」。
 *
...
// 数据自动刷新定时器（§3 步骤 3）：模块加载期即注册、先于 client.connect()；仅 dataRefresh.enabled !== false
...
// 群路由相关配置常量与模块级运行时状态（自 config.json 派生；groups 空数组 = 跟踪全部群）
...
//（wsConnected 断线重连成功不回调本文件、一旦 true 不复位，见 architecture §8 坑 2——此处不修）
