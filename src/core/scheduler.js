...
// 依赖：./logger.js（log/err）；唯一实例化点 src/index.js：new Scheduler(config.schedule || {})，启动时 start(dailyReport)、进程退出时 stop()。
// 数据：不读写任何文件；触发时刻由实例化方传入（来自 config.json 的 schedule.dailyHour / schedule.dailyMinute，缺省 9:00）。
