...
> 代码已按「core 主运行库 + plugins」重构（P0–P3，2026-09，见 [refactor-proposal.md](refactor-proposal.md)）；文中行号仅核心文件引用，改动后可能漂移。
...
## 2. 目录与模块职责（重构后：core 主运行库 + plugins）

```
src/            Node ESM；入口 src/index.js（引导 17 行：import { main } 按入口判定执行）
  core/         主运行库——平台/公共服务 + 注册表 + 唯一装配者（原 src/ 平铺模块迁入）
    runtime.js   createApp(config, overrides) 纯装配 + 消息路由链（S1–S7/S9/S10）+ start/stop
                 生命周期 + backfillHistory + main()（读 config、LLM_API_KEY 校验）
...
模块行数规模（2026-09，含注释/空行）：src/ 合计约 4.9k 行——core/ ≈3.4k（runtime.js ≈430 行为核心装配与路由）、plugins/ ≈1.6k。
...
5. WS open 后 NapCat 合成 `lifecycle/connect` 事件 → `ready=true`/wsConnected → 回填 selfId（若 0）→ `backfillHistory()`（仅一次）。
6. `stop()`（信号路径与测试共用）：`registry.stopAll()`（priority 逆序：webui 关面板 → report 停调度 → refresh 清定时器）→ `client.close()`。

## 4. 消息路由链（core 判定段 S1–S7/S9/S10 + 插件分发带）

收到任意 WS 事件后按序执行；**一旦命中即 return**。core 判定段全部在 runtime.js 的 onEvent 回调内；
原 S8/S11/S12/S13 的产品行为已迁插件，由分发带承接（priority 降序、首个 string/true 短路）。
S# 编号保留为行为契约锚点（CLAUDE.md 红线与 refactor-proposal 保真清单沿用同一套叫法）。

| # | 判定（runtime.js） | 命中行为 | 未命中流向 |
|---|---|---|---|
| S1 | `meta_event` 且 `lifecycle/connect` | `ready/wsConnected=true`；回填 selfId（若 0）；异步 `backfillHistory()`（仅一次） | ↓ |
...
| S12 | **插件分发带** `registry.dispatch(ctx)`（text = 剥 @ 后的问题文本） | 见下方分发表；string → runtime 发送 | — |
...
分发返回值处理（runtime S12 段）：`string` → runtime 群发该文案（`auto_escape: true`、群内 at 为文本模拟，旧语义不变）并记指令日志；`true` → 插件自驱（总结/刷新/chat 均异步进行、本层不发送）。chat 插件恒 true 消费 → **dispatch 不再有落空路径**——原「严格 null 才落 chat」边界内化为带末端（行为不变：LLM 兜底仍最晚执行、仍不 await、brain 内失败回退 defaultReply 文案照发）。report/webui 插件 priority 0，不占任何带（handleMessage 恒 null，仅 hooks）。
...
- **关键词判定基准（P3b 差异，见 §8 坑 11）**：手动总结关键词在分发带内对**剥 @ 后的问题文本**判（旧 S8 对含 @ 的完整文本判、且先于剥 @）。常规「@机器人 总结」（带空格或 @ 段后另起文本）两者等价；差异只出现在**无空白紧贴 @ 的一整串**（如文本「@PRTS总结」、at 段 qq 形态「@10001总结」且 @ 段 name 缺失）——旧代码整串 contains 关键词即触发总结，现代码整串被 extractQuestion 前导正则吞掉 → 落 S10 空 @ 提示。剥 @ 后关键词仍完整时（如「@机器人总结」被 `^@机器人\s*` 单独剥掉）不受影响。
...
### 6.1 离线补偿 backfillHistory（core/runtime.js，connect 生命周期内，仅一次）
...
```
logger ← core 全部服务 + 全部插件（runtime/plugins 都经它记日志）
wiki（纯函数 extractKeywords/isArknightsRelated）← moegirl、wikipedia、chat 插件
store（fmtFull）← summary 插件
registry（PRIORITY 常量）← lingo/ark/gacha/stats/chat/refresh/summary 插件（读带号）
runtime ← core 全部服务 + registry + plugins 全部工厂（唯一装配者）
...
**实例化关系**：`createApp` new 出 MessageStore / NapCatClient / Summarizer / Scheduler / Analytics / DataRefresher 6 个平台服务 + LingoStore / ArkDB / KnowledgeCache / 三个 Wiki 检索器 6 个知识单例，共 12 个实例；`ChatBrain`（注入 6 个知识单例）与 9 个插件描述符随后装配。**重构前最大耦合点「ChatBot 是 lingo/arkdb 等服务宿主、commands/webui 反向借用 chatBot.lingo」已拆**：知识服务现在是 runtime 装配的事实共享单例，brain（字段引用）与指令插件（dispatch ctx）与 webui（getLingo）注入/借用的都是同一批实例（详见 refactor-proposal「服务上移」）。
...
2. **wsConnected 永不复位**：WS 断线自动重连成功后不回调 runtime，`wsConnected` 一旦 true 不再变 false → WebUI 状态页可能显示在线（假象）。napcat 内部重连正常。
...
11. **总结关键词判定基准（P3b 插件化引入的差异，勿当 bug 顺手修）**：旧 S8 在剥 @ 前对**含 @ 的完整文本**判关键词（先于空 @ 判定），现 summary 插件对**剥 @ 后问题文本**判（在 S10 之后）——详见 §4 要点。触发差异的输入形态：无空白紧贴 @ 的一整串且整串含关键词（如文本「@PRTS总结」、at 段缺 name 时「@10001总结」），旧代码触发手动总结、现代码回「艾特PRTS干什么呀喵」。修复方向（需立项讨论）：把关键词判定基准改回剥 @ 前的完整文本（涉及 S8 与 S10 的次序/数据面调整），或调整 extractQuestion 的「段」边界语义——均触及行为红线，勿单方面改。
