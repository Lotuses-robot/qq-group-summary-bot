...
> 2026-09 重构后代码按 core/（主运行库：顶层 runtime/registry/routing 装配与判定层 + platform/ 服务 + knowledge/ 知识单例）与 plugins/（功能插件）分目录；
> 下表「消费方」以类名/函数名标注，归属层写在其后（如「ChatBrain（plugins/chat.js）」「routing」= core/routing.js 的 S 链判定段；P5 起 S 链已从 core/runtime.js 拆出）。
...
| 键 | 类型 | 默认 | 消费方 | 说明 |
|---|---|---|---|---|
| `napcat.wsUrl` | string | 无（缺失即异常） | NapCatClient（core/platform/napcat.js） | 正向 WS 地址，如 `ws://127.0.0.1:3001` |
...
| 键 | 类型 | 默认 | 消费方 | 说明 |
|---|---|---|---|---|
| `llm.apiKey` | string | env `LLM_API_KEY` | Summarizer（core/platform/summarizer.js）+ ChatBrain（plugins/chat.js） | 两构造器各自再兜底 env |
...
| `llm.arkdbDir` | string | `<root>/data/ark` | ArkDB（core/knowledge/arkdb.js） | 方舟数据目录 |
| `llm.wikiEnabled` | boolean | `true` | WikiRetriever（core/knowledge/wiki.js） | PRTS.Wiki 检索开关（`!== false`） |
...
| `llm.moegirlEnabled` | boolean | `true` | MoegirlRetriever（core/knowledge/moegirl.js） | |
...
| `llm.wikipediaEnabled` | boolean | `false` | WikipediaRetriever（core/knowledge/wikipedia.js） | **opt-in**（`=== true` 才开）；需代理访问 zh.wikipedia.org |
