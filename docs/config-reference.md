...
> 2026-09 重构后代码按 core/（主运行库，含 runtime 路由链）与 plugins/（功能插件）分目录；
> 下表「消费方」以类名/函数名标注，归属层写在其后（如「ChatBrain（plugins/chat.js）」「runtime」= core/runtime.js 路由链）。
...
| 键 | 类型 | 默认 | 消费方 | 说明 |
|---|---|---|---|---|
| `napcat.wsUrl` | string | 无（缺失即异常） | NapCatClient（core/napcat.js） | 正向 WS 地址，如 `ws://127.0.0.1:3001` |
| `napcat.selfId` | number | `0` | runtime（S3 过滤/connect 回填起点） | 机器人 QQ 号；为 0 时连接后经 `get_login_info` 回填 |
| `napcat.accessToken` | string | `''` | NapCatClient | 拼到 WS URL 的 `?access_token=` |
| `groups` | number[] | `[]` | runtime | 监控群白名单；**空数组 = 全部群**（还影响 backfill/日报/播报的目标群集合） |
| `schedule` | object | `{}` | Scheduler（core/scheduler.js） | ⚠️ **死配置**：只解构 `dailyHour/dailyMinute`，而 example 里是 `hour/minute`——日报实际恒 9:00（见 architecture §8 坑 1） |
...
| `quiet.enabled` | boolean | `true` | runtime（S7） | 静默时段开关 |
| `quiet.start` / `quiet.end` | number | `0` / `8` | runtime（S7） | 小时制 `[start, end)`；start>end 视为跨零点 |
| `minMessages` | number | `1` | summary 插件（doSummary） | 手动总结低于该消息条数时跳过 |
| `includeSelf` | boolean | `false` | runtime（S3） | `true` 时机器人自己的消息也计入 |
| `commands.manualSummary` | string[] | `['总结','/总结','#总结']` | summary 插件（分发带 900） | 手动总结触发关键词（剥 @ 后问题文本 includes 判定） |
| `backfill.maxHours` | number | `72` | runtime backfillHistory | lastSeen 兜底上限；实际从 lastSeen 时刻起拉 |
...
| `filter.enabled` | boolean | `true` | runtime（filterMessages 包装） | 敏感内容过滤开关（总结/日报前） |
...
## llm.* 块（一个块喂给 Summarizer 与 ChatBrain 两个消费者）

| 键 | 类型 | 默认 | 消费方 | 说明 |
|---|---|---|---|---|
| `llm.apiKey` | string | env `LLM_API_KEY` | Summarizer（core/summarizer.js）+ ChatBrain（plugins/chat.js） | 两构造器各自再兜底 env |
...
| `llm.maxTokens` | number | **Summarizer `2048` / ChatBrain `1024`（默认不同！）** | 两者 | 见 architecture §8 坑 6 |
...
| `llm.lingoFile` | string | `<root>/data/lingo.json` | LingoStore（core/lingo.js） | 相对路径按 cwd 解析（与 arkdbDir/cacheFile 一致） |
| `llm.cacheFile` | string | `<root>/data/knowledge_cache.json` | KnowledgeCache（core/cache.js） | 同上 |
| `llm.cacheTtlHours` | number | `168` | KnowledgeCache | 缓存 TTL；命中不续期 |
| `llm.arkdbDir` | string | `<root>/data/ark` | ArkDB（core/arkdb.js） | 方舟数据目录 |
| `llm.wikiEnabled` | boolean | `true` | WikiRetriever（core/wiki.js） | PRTS.Wiki 检索开关（`!== false`） |
...
| `llm.moegirlEnabled` | boolean | `true` | MoegirlRetriever（core/moegirl.js） | |
...
| `llm.wikipediaEnabled` | boolean | `false` | WikipediaRetriever（core/wikipedia.js） | **opt-in**（`=== true` 才开）；需代理访问 zh.wikipedia.org |
